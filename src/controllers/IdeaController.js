...
import IdeaComment from "../models/IdeaComment.js";
...
class IdeaController {
  static async showIdeas(req, res) {
    ...
    const ideaListWithPagination = await Idea.getListWithPagination(
      currentPage,
      search,
      User,
    );
    ...
    ideasList.forEach((element) => {
      IdeaLike.findOne({ where: { ideaId: element.id, userId: userId } }).then(
        (like) => {
          if (like) {
            element.liked = true;
          }
  ...
  static async deleteIdea(req, res) {
    ...
    if (
      idea.userId != req.session.userId &&
  ...
  static async likeIdea(req, res) {
    try {
      ...
      const ideaLike = await IdeaLike.findOne({
  ...
  static async showIdea(req, res) {
    ...
    try {
      ...
      if (!idea) {
        req.flash("message", "Ideia não encontrada!");
        return res.redirect("/");
      }

      const liked = await IdeaLike.findOne({
        where: { ideaId: ideaId, userId: userId },
      });

      idea.liked = !!liked;

      res.render("idea/show", { idea: idea.get({ plain: true }) });
    } catch (error) {
      logger.error("Erro ao buscar ideia:", error);
      req.flash("message", "Erro ao buscar ideia");
      res.redirect("/");
    }
  }
}
